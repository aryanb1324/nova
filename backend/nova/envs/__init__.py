...
def register_task(
    ...
    *,
    ...
    `requires` states what a robot must provide to be trainable on this task -
    `{"sites": (...), "mocap_bodies": (...), "free_joints": (lo, hi), "note": ...}`.
    It is what lets an uploaded robot be matched to tasks automatically, so a
    task that reads a named site should say so here or it will appear to accept
    robots that cannot actually run it.
    """
...
register_task(
    ...
    requires={
        ...
        "mocap_bodies": ("target",),
        "free_joints": (0, 0),
        "note": "needs a site named 'ee' at the fingertip and a mocap body named "
                "'target'; the arm itself must be bolted down",
    },
)
register_task(
    "locomotion",
    ...
    requires={
        "free_joints": (1, 1),
        "note": "needs exactly one free joint, so the robot can travel",
    },
)
...
def get_task(key: str) -> dict:
    try:
        ...
        # Same reason as robots.get(): a spawned training worker never ran the
        # registrations the parent did. Recover from the environment, once.
        from ..robots import _recover

        if _recover() and key in TASKS:
            return TASKS[key]
