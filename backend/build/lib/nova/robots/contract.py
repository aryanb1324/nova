...
#: Per-task interface. A custom body has to provide these to be trainable.
TASK_REQUIREMENTS = {
    "reach": {
        "sites": ("ee",),
        "mocap_bodies": ("target",),
...
def _check_task(model, task: str, report: Report) -> None:
    ...
    if spec is None:
        report.fail(f"unknown task {task!r}; have {sorted(TASK_REQUIREMENTS)}")
        return

    for site in spec["sites"]:
        if mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, site) < 0:
            report.fail(f"task {task!r} needs a site named {site!r} - {spec['note']}")

    for body in spec["mocap_bodies"]:
        bid = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, body)
        if bid < 0:
            report.fail(f"task {task!r} needs a body named {body!r} - {spec['note']}")
        elif model.body_mocapid[bid] < 0:
            report.fail(f"body {body!r} must be declared mocap=\"true\"")
    ...
    if not (low <= free <= high):
        report.fail(
            f"task {task!r} expects {low if low == high else f'{low}-{high}'} free "
            f"joint(s), found {free} - {spec['note']}"
        )
